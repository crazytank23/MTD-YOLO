...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        #WCAM生成权重-------------------
        #self.w_avg = nn.Parameter(torch.ones(in_planes, dtype=torch.float32), requires_grad=True)
        #self.w_max = nn.Parameter(torch.ones(in_planes, dtype=torch.float32), requires_grad=True)
    ...
    def forward(self, x):
        #WCAM改进通道注意力
        #avg = self.w_avg * self.avg_pool(x)
        #max = self.w_max * self.max_pool(x)
        #c_input = avg + max
        #out = self.f2(self.relu(self.f1(c_input)))
...
class WCAMBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, ratio=16, kernel_size=7):
        super(WCAMBottleneck, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2
...
class WCAM(C3):
    # C3 module with WCAMBottleneck()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)  # 引入C3(父类)的属性
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(WCAMBottleneck(c_, c_, shortcut) for _ in range(n)))
...
class SR(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.conv_l = nn.Conv2d(c1, c1 // 2, 1, bias=False)
        ...
        self.Conv = nn.Sequential(nn.Conv2d((c1 + c2) // 2, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.bn(),
                                  nn.ReLU())
        self._init_weight()

        self.CBR = nn.Sequential(nn.Conv2d((c1 + c2) // 2, 256, kernel_size=7, stride=2, padding=3, bias=False),
                                 # BatchNorm(256),
                                 nn.bn(),
                                 # nn.Dropout(0.5),
        ...
        self.CBRD = nn.Sequential(nn.Conv2d((c1 + c2) // 2, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                  # BatchNorm(256),
                                  nn.bn(),
                                  # nn.Dropout(0.5),
                                  # nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1, bias=False),
        ...
        self.CR = nn.Sequential(nn.Conv2d((c1 + c2) // 2, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                # BatchNorm(256),
                                # nn.bn(),
                                # nn.Dropout(0.5),
                                # nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1, bias=False),
    ...
    def forward(self, x, y, z):  # x is low feature, y is middle feature, z is high feature;
        # x, low_level_feat = input[-2],input[-1]
        x = self.CBR(x)
        z = self.upsample(z)
        total = torch.cat([x, y, z], 1)
        # low_level_feat = self.relu(low_level_feat)
        total = self.CR((self.CR(total)))
        output = EDSR(total)
        # MSELoss函数的具体使用方法如下所示，其中MSELoss函数的参数均为默认参数。
        path, img, img0, self.cap = LoadImages.__next__()
        mse_loss = loss(output, img0)

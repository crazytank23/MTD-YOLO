...
def test(data,
    ...
    for batch_i, (img, ir, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            try:
                out, train_out = model(img,ir,input_mode=input_mode)
            except:
                out, train_out,_ = model(img,ir,input_mode=input_mode)
        ...
        if plots: #and batch_i < 3:
            ...
            # f = '/home/data/zhangjiaqing/dataset/VEDAI/train_label/'+paths[0].split('/')[-1].replace('_co','_label')
            if input_mode == 'IR':

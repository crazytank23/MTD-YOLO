...
from models.yolo import Model #jwl
...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, irs, targets, paths, _) in pbar:  # batch jwl  -------------------------------------------------------------
            ni = i + nb * epoch  # number integrated batches (since train start)
            image = imgs.to(device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0
            ir_image = irs.to(device, non_blocking=True).float() / 255.0 #jwl
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    irs = F.interpolate(irs, size=ns, mode='bilinear', align_corners=False) #jwl

            # Forward
            with amp.autocast(enabled=cuda):
                # t0 = time.time()
                if opt.super:
                    pred,output_sr,_ = model(imgs,irs,opt.input_mode)  # forward #jwl
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str,default='models/mtdyolo_MF_WCAM_BiFPN.yaml', help='model.yaml path') #yolov5s

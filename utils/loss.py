...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        #mse_loss = SR.forward()
        #mse_loss *= self.hyp['cls']
        #loss_ = lbox + lobj + lcls
        #loss_ *= self.hyp['cls']
        loss = lbox + lobj + lcls#loss = mse_loss + loss_
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            indices.append((b, a, gj.clamp_(0, gain[3].long() - 1), gi.clamp_(0, gain[2].long() - 1)))  # image, anchor, grid indices 该网格是哪张图片的，并由哪个锚框进行预测

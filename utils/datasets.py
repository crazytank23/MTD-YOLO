...
def img2label_paths(img_paths):
    ...
    return [x.replace(sa, sb, 1).replace('_' + x.split('_')[-1], '.txt') for x in img_paths] #replace('.' + x.split('.')[-1], '.txt')
def img2ir_paths(img_paths): #jwl    # Define ir image paths as a function of image paths
    return [x.replace('co', 'ir') for x in img_paths] #replace('.' + x.split('.')[-1], '.txt')

def img2img_512_paths(img_paths): #jwl
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        if hr_input== False:
            self.img_path = '/home/data/zhangjiaqing/dataset/VEDAI/images/'
        else:
            self.img_path = '/home/data/zhangjiaqing/dataset/VEDAI_1024/images/'
        ...
        if cache_path.is_file():
            ...
            if cache['hash'] != get_hash(self.label_files + self.img_files + self.ir_files) or 'results' not in cache:  # changed
        ...
        if self.rect:
            ...
            self.ir_files = [self.ir_files[i] for i in irect] #jwl
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        pbar = tqdm(zip(self.img_files, self.label_files, self.ir_files), desc='Scanning images', total=len(self.img_files)) #jwl
        for i, (im_file, lb_file ,ir_file) in enumerate(pbar): #jwl
            try:
                # verify images
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        assert l[0][0]<=7, 'label number is outside' #jwl
        ...
        x['hash'] = get_hash(self.label_files + self.img_files + self.ir_files)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                img2, ir2, labels2 = load_mosaic(self, random.randint(0, self.n - 1)) #jwl
    ...
    def collate_fn(batch):
        img, ir, label, path, shapes = zip(*batch)
...
class LoadImagesAndLabels_sr(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if cache_path.is_file():
            cache = torch.load(cache_path)  # load
            if cache['hash'] != get_hash(self.label_files + self.img_files + self.ir_files) or 'results' not in cache:
                cache = self.cache_labels(cache_path, prefix)  # re-cache
        else:
            cache = self.cache_labels(cache_path, prefix)  # cache
        ...
        # Read cache
        cache.pop('hash')  # remove hash #pop()
        labels, shapes = zip(*cache.values())
        self.labels = list(labels)
        self.shapes = np.array(shapes, dtype=np.float64)
        self.img_files = list(cache.keys())  # update
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        x = {}  # dict
        nm, nf, ne, nc = 0, 0, 0, 0  # number missing, found, empty, duplicate
        pbar = tqdm(zip(self.img_files, self.label_files, self.ir_files), desc='Scanning images', total=len(self.img_files))
        for i, (im_file, lb_file ,ir_file) in enumerate(pbar):
            try:
                # verify images
                im = Image.open(im_file)
                im.verify()  # PIL verify
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        l = np.array([x.split() for x in f.read().strip().splitlines()], dtype=np.float32)  # labels
                    if len(l):
                        assert l[0][0]<=7, 'label number is outside'
                        assert l.shape[1] == 5, 'labels require 5 columns each'
                        assert (l >= 0).all(), 'negative labels'
                        assert (l[:, 1:] <= 1).all(), 'non-normalized or out of bounds coordinate labels'
                        assert np.unique(l, axis=0).shape[0] == l.shape[0], 'duplicate labels'
        ...
        x['hash'] = get_hash(self.label_files + self.img_files + self.ir_files) #jwl
        x['results'] = [nf, nm, ne, nc, i + 1]
        torch.save(x, path)  # save for next time
        logging.info(f'{prefix}New cache created: {path}')
        return x
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, ir, labels = load_mosaic(self, index) #jwl
            #ir = load_ir(self, index) #jwl
            shapes = None
            ...
            if random.random() < hyp['mixup']:
                ...
                r = np.random.beta(8.0, 8.0)  # mixup ratio, alpha=beta=8.0
                img = (img * r + img2 * (1 - r)).astype(np.uint8)
                ir = (ir * r + ir2 * (1 - r)).astype(np.uint8) #jwl
                labels = np.concatenate((labels, labels2), 0)
            ...
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            ir, _, _ = letterbox(ir, shape, auto=False, scaleup=self.augment) #jwl
            shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling
        ...
        if self.augment:
            ...
            # Augment colorspace
            augment_hsv(img, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
            # augment_hsv(ir, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v']) #jwl
    ...
    @staticmethod
    def collate_fn(batch):
        img, ir, label, path, shapes = zip(*batch)  # transposed  #jwl
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.stack(ir, 0), torch.cat(label, 0), path, shapes #jwl
...
def load_mosaic(self, index): #拼接图像
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            ir4 = np.full((s * 2, s * 2, ir.shape[2]), 114, dtype=np.uint8) #jwl
...
def random_perspective(img, ir, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            ir = cv2.warpPerspective(ir, M, dsize=(width, height), borderValue=(114, 114, 114)) #jwl
        else:  # affine
